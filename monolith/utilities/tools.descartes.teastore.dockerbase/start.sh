...
echo 'export JAVA_OPTS="-javaagent:/otel/agent.jar --add-opens=java.base/java.lang=ALL-UNNAMED -Dotel.javaagent.configuration-file=/otel/otel.monitoring.properties -Daj.weaving.verbose=false -Dorg.aspectj.weaver.loadtime.configuration=aop.xml -Daj.weaving.loadersToSkip=java.net.URLClassLoader -Dcom.sun.jndi.ldap.object.disableEndpointIdentification=true"' > /usr/local/tomcat/bin/setenv.sh
